...
//     try {
//         const p = path.resolve(__dirname, '../contracts', 'XRC21.sol')
//         const contractTemplate = fs.readFileSync(p, 'UTF-8')
//         // const newContract = contractTemplate.replace('MyXRC21', name)
//         // return newContract
...
router.post('/createToken', [
    ...
    check('type').exists().withMessage("'type' is required")
        .isIn(['xrc20', 'xrc21']).withMessage("'type' should be 'xrc20' or 'xrc21'"),
    ...
    let p
    if (mintable) {
        p = path.resolve(__dirname, '../contracts', 'XRC21Mintable.sol')
        contractCode = fs.readFileSync(p, 'UTF-8')
    } else {
        p = path.resolve(__dirname, '../contracts', 'XRC21.sol')
        contractCode = fs.readFileSync(p, 'UTF-8')
    }
    return res.json({
        name,
...
router.post('/compileContract', [
    ...
    try {
        ...
        if (mintable) {
            contract = compiledContract.contracts['MyXRC21Mintable'] ||
                compiledContract.contracts[':' + 'MyXRC21Mintable']
        } else {
            contract = compiledContract.contracts['MyXRC21'] || compiledContract.contracts[':' + 'MyXRC21']
        }
...
router.get('/getABI', [], async (req, res, next) => {
    try {
        ...
        if (type === 'mintable') {
            p = path.resolve(__dirname, '../build/contracts', 'MyXRC21Mintable.json')
            ...
            p = path.resolve(__dirname, '../build/contracts', 'MyXRC21.json')
            const data = fs.readFileSync(p, 'UTF-8')
            if (data) {
                return res.json({
                    abi: JSON.parse(data).abi
...
router.get('/search', [], async (req, res, next) => {
    try {
        ...
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/tokens/search?${serializeQuery(callQuery)}`)
        )
...
router.get('/:token', [], async (req, res, next) => {
    try {
        ...
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/tokens/${token}`)
        )
        return res.json(data)
    } catch (error) {
        return next(error)
...
router.get('/holders/xrc21/:token', [], async (req, res, next) => {
    try {
        const token = req.params.token || ''
        const page = req.query.page || 1
        const limit = req.query.limit || 20
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/token-holders/xrc21/?address=${token}&page=${page}&limit=${limit}`)
        )
        return res.json(data)
    } catch (error) {
        return next(error)
...
router.get('/holders/:token', [], async (req, res, next) => {
    try {
        ...
        const limit = req.query.limit || 20
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/token-holders/?address=${token}&page=${page}&limit=${limit}`)
        )
        return res.json(data)
    } catch (error) {
        return next(error)
...
router.get('/txes/xrc20/:token', [], async (req, res, next) => {
    try {
        const token = req.params.token || ''
        const page = req.query.page || 1
        const limit = req.query.limit || 20
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/token-txs/xrc20?token=${token}&page=${page}&limit=${limit}`)
        )
        return res.json(data)
    } catch (error) {
        return next(error)
...
router.get('/txes/xrc21/:token', [], async (req, res, next) => {
    try {
        const token = req.params.token || ''
        const page = req.query.page || 1
        const limit = req.query.limit || 20
        const { data } = await axios.get(
            urljoin(config.get('xinfinscanAPI'), `/api/token-txs/xrc21?token=${token}&page=${page}&limit=${limit}`)
        )
        return res.json(data)
    } catch (error) {
        return next(error)
