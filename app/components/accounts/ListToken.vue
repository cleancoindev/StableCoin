<template>
    <div
        ...
        <div class="tmp-table-one other-dots">
            <b-tab
                ...
                <template>
                    <div
                        class="xinfin_main_table colum-8">
                        <b-table
                            id="transfer_table"
                            :items="listokenItems"
                            ...
                            <template
                                ...
                                slot-scope="data">
                                <b-link
                                    :href="config.xdcwalletUrl + '/xrc21/' + data.item.hash"
                                    target="_blank">
                                    Transfer Token
                                </b-link>
                            </template>
                            <template
                                ...
                                <b-dropdown
                                    ...
                                    <template
                                        slot="button-content">
                                        <i class="stablecoin-three-dots" />
                                    </template>
                                    <b-dropdown-item
                                        v-if="!data.item.applyxinfinz"
                                        :to="'/xinfinzcondition/' + data.item.hash">
                                        Apply to XinFInZ
                                    </b-dropdown-item>
                                    <b-dropdown-item
                                        v-if="!data.item.applyxinfinx"
                                        :to="'/xinfinxcondition/' + data.item.hash">
                                        Apply to XinFinX
                                    </b-dropdown-item>
                                    <b-dropdown-item
    ...
    <div
        ...
        <div class="xdc-body-fullw text-center">
            <p>
                <b-img
...
export default {
    ...
    data () {
        return {
            ...
            listokenFields: [
                ...
                { key: 'applyxinfinz', label: '', variant: 'sp-text-center' }
            ],
            listokenItems: [],
    ...
    methods: {
                ...
                if (data.items.length > 0) {
                    const map = await Promise.all(data.items.map(async i => {
                        return {
                            ...
                            holders: i.holders || '---',
                            applyxinfinz: ((self.appliedZList || []).indexOf(i.hash) > -1),
                            applyxinfinx: ((self.appliedXList || []).indexOf(i.hash) > -1),
        ...
        async checkAppliedZ () {
            const contract = await this.XRC21Issuer
        ...
        async checkAppliedX () {
            const contract = await this.XinFinXListing
            const result = await contract.methods.tokens.call()
            if (result && result.length > 0) {
                let lowerCaseArr = result.map(m => m.toLowerCase())
                return lowerCaseArr
