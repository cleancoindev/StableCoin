<template>
    <div class="container">
        <div
            v-if="isAppliedZ"
            class="mt-3"><b>This token is already applied to XinFInZ</b></div>
        <div
            v-if="isAppliedX"
            class="mt-3"><b>This token is already applied to xinfinx</b></div>
        <div
            v-if="!isAppliedZ">
            <b-form-group
                id="deposite"
            ...
            <b-button
                variant="primary"
                @click="applyXinFinZ">ApplyZ</b-button>
        </div>
        <div
            v-if="!isAppliedX"
            class="mt-3">
            ...
            <b-button
                variant="primary"
                @click="applyXinFinX">ApplyX</b-button>
        </div>
        <div
            v-if="loading"
            class="mt-5">
...
export default {
    ...
    data () {
        return {
            ...
            loading: false,
            txFee: '',
            issuerContract: this.XRC21Issuer,
            xinfinxContract: this.XinFinXListing,
            config: {},
            token: {}
        }
    },
    ...
    methods: {
        ...
        async applyXinFinX () {
            try {
                this.loading = true
                const xinfinxContract = this.xinfinxContract
                const txParams = {
                    from: (await this.getAccount()).toLowerCase(),
        ...
        async checkAppliedX () {
            const contract = this.xinfinxContract
            if (contract) {
                const result = await contract.methods.tokens().call()
                if (result && result.length > 0) {
                    const lowerCaseArr = result.map(m => m.toLowerCase())
