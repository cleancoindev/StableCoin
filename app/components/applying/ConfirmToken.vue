<template>
    <div class="container container-small flex-content-center">
        <div class="confirm-table xdc-body-fullw-issue issue-confirm">
            <div class="info-header">
                <h2 class="tmp-title-large m-0">StableCoin Issuance Review</h2>
                <p class="text-center mt-5"><i class="stablecoin-icon-startup"/></p>
            ...
            <div class="tmp-table-two grid-two issue-confirm">
                <table>
                    <tr>
                        <td>StableCoin Name</td>
                        <td>{{ tokenName }}</td>
                    </tr>
                    <tr>
                        <td>StableCoin Symbol</td>
                        <td>{{ tokenSymbol }}</td>
                    </tr>
                    <tr>
                        <td>StableCoin Supply</td>
            ...
            <b-modal
                ...
                <div class="xdc-modal-default">
                    <div class="msg-txt">
                        <i class="stablecoin-icon-checkmark-outline"/>
                        <h4>Successful</h4>
                        <p>StableCoin successfully issued</p>
                        <p>
                            Transaction hash:
                            <a
        ...
        async createContract () {
            ...
            try {
                ...
                if (tokenContract && tokenContract.data) {
                    ...
                    // console.log(`Here is Source code from CT ${self.sourceCode}`)
        ...
        async deploy () {
            ...
            try {
                ...
                if (self.balance.isLessThan(self.txFee)) {
                    self.loading = false
                    self.$toasted.show('Not enough XDC', { type: 'error' })
                } else {
                    ...
                    case 'trezor':
                        ...
                        if (txHash) {
                            ...
                            while (check) {
                                const receipt = await web3.eth.getTransactionReceipt(txHash)
                                if (receipt) {
                                    if (receipt.contractAddress.substring(0, 3) === 'xdc') {
                                        self.contractAddress = '0x' + receipt.contractAddress(3)
                                    }
                                    setTimeout(() => {
                                        self.loading = false
                                        check = false
